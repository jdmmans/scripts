...
# remove isolate names, list and extract factors for the metadata names, change all absent to black, and all present to white  
ab<- subset(ab, select= -c(Isolate))
c<- names(ab)
ab[ab == "0"] <- "black" 
ab[ab == "1"] <- "white"
# metadata + phylogeny being plotted together
par(mar = c(12,8,1,1))
plot(cPhylo1)
colored_bars(ab, dend = cPhylo1, rowLabels = c)
```
